...
- `Rozliczenia` is the canonical billing module. `/rozliczenia` is the product route; `/kasa` is only a legacy redirect and must not be developed as a separate screen.
...
| Rozliczenia | `/rozliczenia` | Canonical read-only billing ledger view | Yes, `GET /api/billing/ledger/balances` | Yes | Yes | Yes | No | `test-billing.js` | Use only `/rozliczenia` for product work; `/kasa` is a legacy redirect, not a separate module. Next step should be contract audit before any payment action. |
