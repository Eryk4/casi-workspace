...
- `Rozliczenia` is the canonical billing module. `/rozliczenia` is the product route and now hosts `Centrum rozliczeń` product v1; `/kasa` is only a legacy redirect and must not be developed as a separate screen. V1 is a read-only foundation for a future full settlement module covering clients, students, families, payers, services, enrollments, pricing, discounts, periods, charges, payments, allocations, arrears, reminders, owner reports, and accounting export. The current services/enrollments, billing-period, and payments/allocation foundations are inferred from existing charges, models, invoices, contractors, transactions, and matched payments; the full target domain plan is documented in `docs/FULL_CLIENT_BILLING_DOMAIN_PLAN.md`.
...
- Rozliczenia, including product v1 `Centrum rozliczeń` at `/rozliczenia`, payer detail at `/rozliczenia/platnicy/[payerId]`, billing periods at `/rozliczenia/okresy`, and payments/allocation read-only view at `/rozliczenia/wplaty`
