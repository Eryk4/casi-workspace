...
| Rozliczenia | `/rozliczenia` | Product v1 billing center for money, payments, receivables, contractors, invoices, and related matters; includes read-only payer/family/student foundation, balance explanations, and payer detail | Yes, billing ledger balances, billing charges, invoices, contractors, and Work Items | Yes | Yes for all current sources | Yes | No | `test-billing.js` | Use `docs/FULL_CLIENT_BILLING_DOMAIN_PLAN.md` for the target module direction. Recommended next implementation after payer detail: read-only charge or service context, with no payment writes. |
