...
W v1 zarejestrowanych jest sześć niezależnych subsystemów: `internal_notification_scheduler`, `task_reminders`, `knowledge_processing`, `email_import`, `ksef_import` oraz `automation_engine`. Centrum nie przepina pięciu wyspecjalizowanych źródeł do `automation_rules` i nie tworzy drugiego silnika wykonawczego.
...
| `automation_engine` | Reguły automatyzacji | organizacja | aktywne rekordy `automation_rules` | `automation_executions`; payloady są pomijane | ostatnie wykonanie aktywnej reguły | `unknown`; brak heartbeat/TTL/lease | brak produkcyjnego widoku ustawień | wdrożony read-only |
...
`automation_rule` reprezentuje organizacyjną definicję dopasowania typu zdarzenia, warunków i listy akcji. Jedynym trwałym stanem konfiguracji jest `is_active`, prezentowany jako enabled/disabled. `automation_execution` jest trwałym zapisem zakończonej próby obsługi jednego dopasowanego eventu przez regułę. Obecny writer zapisuje wyłącznie `success` albo `failed`; nie istnieją statusy pending, running ani partial success.

Silnik jest event-driven. Ręczne/API `run` przetwarza oczekujące eventy, ale nie tworzy wykonania bez dopasowanego zdarzenia. Nie ma źródła scheduled ani automatycznie uruchamianej pętli/worker callera. Dwie jawne akcje to `create_inbox_item` i `log`; typy są sprawdzane przez centralny dispatcher dopiero podczas wykonania. Wyspecjalizowane subsystemy Centrum nie korzystają z generic engine.

Reguła ma organizacyjny scope; `created_by_user_id` jest metadanymi, a nie zakresem widoczności. Centrum nie pokazuje reguł ani wykonań innych organizacji. Nie eksponuje nazwy, opisu, trigger type, conditions, actions, input/result JSON, event payloadu ani surowego `error_message`. Nazwa UI jest neutralna (`Reguła #ID`), a błąd ma stały kod i polskie, sanitarne podsumowanie.

Health jest deterministyczny: brak aktywnych reguł daje `disabled`; aktywne reguły bez wykonania dają `never_run`; ostatnie wykonanie aktywnej reguły `success` daje `healthy`, a `failed` daje `attention`. Wiek wykonania nie tworzy SLA. Brak heartbeat, lease i TTL oznacza zawsze `runtime_status=unknown`.
...
„Ostatnia aktywność” jest neutralną, historyczną listą zakończonych działań. W odróżnieniu od „Wymaga uwagi” może zawierać wiele wpisów jednego adaptera oraz sukcesy, częściowe sukcesy i błędy. Historyczny błąd pozostaje na liście po późniejszym sukcesie; sekcja nie jest inboxem, monitoringiem runtime ani cyklem życia incydentu.

Osobny read-only endpoint `GET /api/automations/operations/activity` zwraca `{ items, limit }`. Domyślny limit wynosi 8, jawny zakres to 1–20, a błędne wartości zwracają 400. Endpoint nie ma paginacji, cursora ani utrwalonej wspólnej tabeli i może być później użyty przez główny dashboard bez rozszerzania bazowego `GET /api/automations/operations`.

Każdy wpis ma wyłącznie `activity_id`, `automation_key`, tytuł z centralnego registry, `activity_type`, wspólny `status`, kanoniczny `occurred_at`, kontrolowane `summary` oraz istniejący `details_url`. Identyfikator ma postać `<automation_key>:<source_type>:<source_id>`. Typy to `scheduled_check`, `delivery`, `processing`, `import` i `execution`, a statusy: `succeeded`, `failed` i `partial`. Czas jest poprawnym ISO-8601 UTC z jawną strefą; niepoprawne, puste i naive timestamps są pomijane. Historyczne naive `task_reminder_outbox_attempts.attempted_at` pozostają nietknięte i nie kwalifikują się do wspólnego porządku.

Źródłami jest dokładnie sześć istniejących adapterów. Scheduler uwzględnia zakończone błędy oraz sukcesy z rzeczywistym efektem i zachowuje scope odbiorcy. Task Reminders uwzględnia `sent`, `failed` i `dead_letter` z zachowaniem owner/assignee/explicit visibility. Knowledge Processing używa terminalnych `completed`/`failed`. Import e-maili i KSeF używa `completed`, `completed_with_issues` i `failed`, a Automation Engine `success`/`failed`. Pending, running, retry, skipped, no-op, watcher i heartbeat są wykluczone. Teksty KSeF jawnie mówią o „lokalnym uruchomieniu procesu importu KSeF”, ponieważ integracja nadal jest `mvp_stub`.

Każdy adapter wykonuje jedno ograniczone zapytanie top-N po własnym trwałym źródle. Wyniki są mapowane przez adapter, a następnie łączone i sortowane centralnie po `occurred_at DESC`, `automation_key ASC`, `source_type ASC`, `source_id DESC`. Istniejące addytywne indeksy obsługują te zapytania; sekcja nie używa `event_logs`, nie pobiera item-level danych i nie wykonuje N+1.

Odpowiedź korzysta z twardej allowlisty i stałych polskich szablonów. Nie ujawnia nazw ani treści dokumentów, danych e-maili i zadań, danych faktur/KSeF, reguł i payloadów silnika, surowych błędów, ścieżek ani sekretów. Dozwolone są wyłącznie bezpieczne liczniki zapisane już na poziomie runu. GET nie tworzy activity, notification ani event logu i nie uruchamia procesów; pełne uwierzytelnione HTTP zachowuje wyłącznie niezależny, istniejący touch `user_sessions.last_seen_at`.
