...
### Stage 2m: Internal notifications center

Status: implemented as `notifications v1` at `/powiadomienia`.

The stage introduces a durable, recipient-scoped inbox for the currently authenticated `users.user_id`. Notifications are isolated by both `organization_id` and `recipient_user_id`. Explicit materialization reuses the central billing next-step attention read-model and snapshots only overdue and due-today signals. One source event can produce distinct `due_today` and later `overdue` notifications, while the database deduplicates the same organization, recipient, source event, and reason under concurrent calls. Two identical next steps remain separate because their event IDs remain separate.
